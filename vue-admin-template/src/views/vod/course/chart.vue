...
export default {
  ...
  created() {
    ...
    this.startDate = this.dateFormat(new Date(currentDate.getTime() - 365 * 24 * 3600 * 1000))
    this.endDate = this.dateFormat(currentDate)
    this.showChart()
  },
  methods: {
    showChart() {
      videoVisitorApi.findCount(this.courseId, this.startDate, this.endDate).then(response => {
