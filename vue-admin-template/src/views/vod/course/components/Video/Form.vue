...
export default {
  ...
  methods: {
    ...
    handleBeforeUpload(file) {
      console.log("上传前")
      const uploadLimit = 2
      const uploadTypes = ['mp4', 'jpg', 'png', 'doc', 'docx', 'xlsx', 'zip', 'rar', 'pdf']
      const filetype = file.name.replace(/.+\./, '')
      const isRightSize = (file.size || 0) / 1024 / 1024 / 1024 < uploadLimit
      if (!isRightSize) {
        this.$message.error('文件大小超过 ' + uploadLimit + 'GB')
        return false
      }

      if (uploadTypes.indexOf(filetype.toLowerCase()) === -1) {
        this.$message.warning({
          message: '请上传后缀名为mp4,jpg、png、txt、pdf、doc、docx、xlsx、zip或rar的附件'
        })
        return false
      }
      return true
    ...
    handleUploadSuccess(response, file, fileList) {
      ...
      this.video.videoSourceId = strings[strings.length - 1]
